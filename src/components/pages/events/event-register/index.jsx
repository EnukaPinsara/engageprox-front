...
const EventRegister = () => {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        const { eventName, eventLocation, registrationDeadline } = response.data;
        setEventData({
          eventName: eventName,
  ...
  const handleRegister = async () => {
    const participationData = {
      ...
      hasParking: parkingNeed,
      vehicleType: 'example-vehicle',
      vehicleNumber: 'example-vehicle-number',
      invitationAccepted: isInviteConfirm,
      reasonNotAccepted: reason
    };

    try {
      const response = await axios.post('/api/eventParticipation', participationData);
      console.log('Event participation saved:', response.data);
    } catch (error) {
    ...
    <>
      ...
      <Card className="my-3">
        ...
        <Card.Body className="bg-body-tertiary">
          ...
          <div className="mb-3">
            <strong>Event Deadline:</strong> {eventData.registrationDeadline}
          </div>
        </Card.Body>
      </Card>
